...
public interface UserRepository extends JpaRepository<UserInfo,Integer>, JpaSpecificationExecutor<UserInfo> {
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO sys_users_roles VALUES(?1, ?2)", nativeQuery = true)
    int saveroles(Integer userId, Integer roleId);

    //给用户删除角色
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM sys_users_roles WHERE user_id = ?1 and role_id = ?2", nativeQuery = true)
